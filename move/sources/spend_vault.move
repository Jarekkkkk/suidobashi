// Vendored from OpenZeppelin, not written here.
//
// move/Move.toml records the source repository, the exact revision this was taken
// from, and the single line that was changed (the module declaration). The rest is
// verbatim, including the bearer-cap warning below, which is the rule that
// move/sources/policy.move honours when it embeds a SpenderCap inside a Policy.
//
// This note is here rather than only in Move.toml so the provenance travels with
// the file if it is ever read on its own.
